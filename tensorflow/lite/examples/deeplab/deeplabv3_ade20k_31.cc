...
int main(int argc, char *argv[]) {
  ...
  // accumulators for true positives, false positives, and false negatives
  std::vector<uint64_t> tp_acc, fp_acc, fn_acc;

  // assuming 2,000 images
  const int num_images = 2000;
  for (int i = 1; i <= num_images; i++) {
    string ground_truth_prefix =
        "/data/local/tmp/ade20k_512/annotations/raw/ADE_val_0000";
    string input_bitmap_prefix =
        "/data/local/tmp/ade20k_512/images/bmp/ADE_val_0000";
  ...
  std::cout << "mIOU over_all: " << iou_sum / NUM_CLASSES << "\n";
